...
?>
        <tr>
            ...
            <td><a href="viewprofile.php?id=<?php echo $user_id; ?>"class = "button">View Profile</a></td>
            <td><?php echo $user_position; ?></td>
            <td><?php echo $user_email; ?></td>
            <td>
                <?php if ($user->get_user_status($id) == "1") { ?>
                    <a href="processes/process.user.php?action=delete&id=<?php echo $user_id; ?>" class="button" onclick="return confirm('Are you sure you want to delete this user?')">Delete User</a>
                <?php } else { ?>
                    <a href="processes/process.user.php?action=delete&id=<?php echo $user_id; ?>" class="button" onclick="return confirm('Are you sure you want to delete this user?')">Delete User</a>
                <?php } ?>
            </td>
